 ...
 async with SessionLocal() as s:
  ...
  if not await s.scalar(select(Match).limit(1)):
   ...
   for home,away,kickoff,venue,prices in FIXTURES:
    ...
    s.add(Prediction(match_id=m.id,model_version="ensemble-1.0",probabilities=pred,expected_home_goals=pred["xg_home"],expected_away_goals=pred["xg_away"],score_mode=pred["score"],features={"seed":True}))
