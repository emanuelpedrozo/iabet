...
class Odd(Base, TimestampMixin):
    __tablename__="odds"; id: Mapped[int]=mapped_column(primary_key=True); match_id: Mapped[int]=mapped_column(ForeignKey("matches.id"),index=True); bookmaker: Mapped[str]=mapped_column(String(60),index=True); market: Mapped[str]=mapped_column(String(80),index=True); selection: Mapped[str]=mapped_column(String(100)); line: Mapped[float|None]=mapped_column(Float); price: Mapped[float]=mapped_column(Float); captured_at: Mapped[datetime]=mapped_column(DateTime(timezone=True),index=True); match: Mapped[Match]=relationship(back_populates="odds"); __table_args__=(Index("ix_odds_lookup","match_id","market","selection","captured_at"),)
class Prediction(Base, TimestampMixin):
    __tablename__="predictions"; id: Mapped[int]=mapped_column(primary_key=True); match_id: Mapped[int]=mapped_column(ForeignKey("matches.id"),index=True); model_version: Mapped[str]=mapped_column(String(50)); probabilities: Mapped[dict]=mapped_column(JSON); expected_home_goals: Mapped[float]=mapped_column(Float); expected_away_goals: Mapped[float]=mapped_column(Float); score_mode: Mapped[str]=mapped_column(String(10)); features: Mapped[dict]=mapped_column(JSON,default=dict); match: Mapped[Match]=relationship(back_populates="predictions")
